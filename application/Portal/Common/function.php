...
function  return_result($status,$code,$msg,$data){

    	$res = array("status"=>$status,"code"=>$code,"msg"=>$msg,"data"=>$data);
    	echo json_encode($res,true);

}

//检查http请求的sig师傅正确，防止恶意攻击
function check_sign($arr)
{
	$sign = $arr['sign'];
	if(empty($sign))
	{
	...
	foreach($arr as $k=>$v)
	{
		$results[]=sprintf('%s=%s', $k, $v);
	}
	$results[]=sprintf('%s=%s', 'sig', 'jiandanapp');
	$buffer = implode('&', $results);
	
	$calc_sign = md5($buffer);
	if(rtrim($sign)!=$calc_sign)
	{
		return false;
	}
	return true;
